 ...
 * PATHEXT. Looking up the command first preserves the distinction between an
 * absent optional development dependency and a present broken CLI.
 *
...
export function installMergeDrivers(
  ...
  const result = platform === "win32"
    ? runner(
      ...
      {
        env: { ...process.env, PM_CHANGELOG_PM_SHIM: executable },
        shell: false,
        stdio: "inherit",
