 ...
 *
 * Matches {@link ../scripts/sync-version.ts}: `import.meta.url` is already
 * symlink-resolved, so `argv[1]` is resolved through `realpathSync` before
 * comparison. A launcher reaching this file through a symlink (an npm bin shim,
 * a linked workspace) would otherwise compare unequal and skip the gate
 * silently — a release gate that no-ops without erroring is worse than one that
 * throws. `realpathSync` throws on a path that does not exist, so the guard
