...
| `--date <date>` | resolved tag date when available, otherwise current UTC date | Release heading date text; `YYYY-MM-DD` is recommended, and an explicit value is rendered verbatim and overrides the default |
...
| `--dependency-updates` | false | Add a `### Dependencies` section (listed last) to each release window, with one bullet per Dependabot-shaped commit subject (`<type>(deps|deps-dev): bump …`) read from git in that window's tag range; a window with no closed items but such commits still gets its version heading. PR numbers link to GitHub only when `--item-url-base` is a `https://github.com/<owner>/<repo>/…` URL |
