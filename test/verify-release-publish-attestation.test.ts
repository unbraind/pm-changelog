...
test("local composite actions and Dockerfile RUN commands are audited", () => {
  ...
  assert.equal(
    auditPublishAttestation([{ file: "Dockerfile", text: "# release image\nRUN npm publish" }]).failures.length,
    1,
  );
});

test("a trailing escape cannot make shell segmentation read past the source", () => {
...
test("arrays declared in non-executed scopes cannot attest a later publish", () => {
  ...
  for (const [name, declaration] of declarations) {
    ...
    assert.equal(result.failures.length, 1, `${name} array must not provide provenance`);
  }
});
...
test("an unattested publish inside a heredoc body fails the audit", () => {
  const result = auditPublishAttestation([{
    file: "release.yml",
    text: ["cat <<'SCRIPT' > release.sh", "npm publish --access public", "SCRIPT"].join("\n"),
  }]);
  assert.equal(result.failures.length, 1);
  assert.match(result.failures[0]!, /does not enable --provenance/);
});
...
test("an unattested publish inside a subshell fails the audit", () => {
  const result = auditPublishAttestation([{ file: "release.yml", text: "(npm publish --access public)" }]);
  assert.equal(result.failures.length, 1);
  assert.match(result.failures[0]!, /does not enable --provenance/);
});

test("an unattested publish inside an uninvoked function fails the audit", () => {
  const result = auditPublishAttestation([{
    file: "release.yml",
...
test("child-scope assignments cannot leak provenance into a later publish", () => {
  const childScopes = new Map([
    ["command substitution", ["VALUE=$(", "FLAG=--provenance", ")", "npm publish $FLAG"].join("\n")],
    ["subshell", ["(", "FLAG=--provenance", ")", "npm publish $FLAG"].join("\n")],
    ["uninvoked function", ["release() {", "FLAG=--provenance", "}", "npm publish $FLAG"].join("\n")],
  ]);
  for (const [name, text] of childScopes) {
    const result = auditPublishAttestation([{ file: "release.yml", text }]);
