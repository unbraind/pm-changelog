...
describe("docstring-gate: main default roots", () => {
  it("applies the default roots when invoked with no arguments", () => {
    // A fixture with only `src/` exercises the DEFAULT_ROOTS arm of main's
    // ternary: the missing `scripts/` root fails the presence check.
    const dir = mkdtempSync(join(tmpdir(), "docstring-main-defaults-"));
    try {
      mkdirSync(join(dir, "src"), { recursive: true });
      writeFileSync(join(dir, "src", "a.ts"), `${GOOD_DOC}export function parseConfig(): void {}\n`, "utf8");
