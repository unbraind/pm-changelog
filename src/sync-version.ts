...
function syncManifest(manifestPath: string, version: string): void {
 ...
 */
function syncExtensionVersion(extensionPath: string, version: string): void {
  ...
  if (matches.length > 1) {
    throw new Error(
      `Refusing to stamp ${extensionPath}: found ${matches.length} 'version:' literals ` +
