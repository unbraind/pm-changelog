...
function readDependencyCommits(
  gitCwd: string,
  range: { sinceRef?: string; untilRef?: string; sinceTimestamp?: string; untilTimestamp?: string },
): DependencyCommit[] {
  ...
  if (range.sinceRef && range.untilRef) {
    ...
    if (range.sinceTimestamp) args.push(`--since=${range.sinceTimestamp}`);
    if (range.untilTimestamp) args.push(`--until=${range.untilTimestamp}`);
  } else {
    return [];
  }
...
function enrichSectionsWithDependencyCommits(
  ...
  return sections.map((section, index) =>
    index === 0 && commits.length > 0 ? { ...section, dependencyCommits: commits } : section,
  );
}
...
function limitSections(
  ...
  const sinceVersion = options.sinceVersion?.trim()
    ...
    : undefined;
  if (limit === undefined && sinceVersion === undefined) return sections;
  if (!options.releaseWindows || options.releaseWindows.length === 0) return sections;

  let result = sections;
  if (sinceVersion !== undefined) {
    result = result.filter((section) => {
      const key = sectionVersionKey(section.heading);
      if (key === undefined) return true; // keep Unreleased and unparsable headings
...
export class IncompleteListAllError extends Error {
  ...
  constructor(signals: readonly string[], envelope?: Record<string, unknown>) {
    ...
    super(
      `pm list --all --json answer was incomplete and was refused: ${signals.join("; ")}; `
...
/** Collect every receipt signal that makes a whole-tracker `pm list` answer
 * unsafe to consume.
 *
 ...
 * half-fail.
 *
 * @param record - The parsed whole-tracker list envelope.
 * @returns The human-readable signals that tripped, empty when the answer is
 * safe to consume. */
function incompleteListAllSignals(record: Record<string, unknown>): string[] {
  ...
  if (isRecord(record.output_budget_truncation)) {
    ...
    signals.push(
      `output_budget_truncation.reason=${typeof reason === "string" ? reason : "<missing>"}`,
    );
  }
  ...
  const total = typeof record.total === "number" && Number.isInteger(record.total) && record.total >= 0
    ? record.total
    : undefined;
  if (count === undefined) signals.push("count=<missing-or-invalid>");
  if (total === undefined) signals.push("total=<missing-or-invalid>");
...
export function parseListAllItemsJson(raw: string): PmItem[] {
  const parsed = JSON.parse(raw) as unknown;
  ...
  if (signals.length > 0) throw new IncompleteListAllError(signals, parsed);
...
function filterItems(options: GenerateChangelogOptions): PmItem[] {
  if (isDeliberateEmptySuppression(options)) return [];
...
function usesSingleVersionSection(options: GenerateChangelogOptions): boolean {
  if (isInReleaseWindowMode(options)) return false;
...
function normalizeSuppressedReleaseKey(suppressed: string | undefined): string {
 ...
 */
export function compareVersionHeadings(a: string, b: string, fallback: string): number {
...
export function compareVersionStrings(a: string, b: string): number {
  const normalize = (v: string): string => v.replace(/^v/, "");
...
const CONVENTIONAL_HEADINGS: Record<Category, string> = {
  Added: "Features",
  Changed: "Changes",
  Fixed: "Bug Fixes",
  Removed: "Reverts",
...
function groupByField(
  items: PmItem[],
  sectionBy: Exclude<ChangelogSectionBy, "category">
): Array<{ heading: string; items: PmItem[] }> {
  const groups = new Map<string, PmItem[]>();
  const push = (heading: string, item: PmItem): void => {
  ...
  for (const item of items) {
    if (sectionBy === "type") {
      ...
      if (tags.length === 0) {
        ...
        for (const tag of tags) push(tag, item);
      }
    }
  }
 ...
 */
function collectContributors(items: PmItem[]): string[] {
...
function pickContributor(value: unknown): string | undefined {
...
function sampleItems(items: PmItem[]): string[] {
  const labels: string[] = [];
  const seen = new Set<string>();
  for (const item of items) {
    ...
    seen.add(label);
    labels.push(label);
    if (labels.length >= SELECTION_SAMPLE_LIMIT) break;
  }
  return labels;
}
 ...
 */
function formatBodyPreview(item: PmItem, options: GenerateChangelogOptions): string {
 ...
 */
function maybeEmoji(heading: string, options: GenerateChangelogOptions): string {
  if (!options.emojiPrefix) return heading;
 ...
 */
export function explainChangelogSelection(options: GenerateChangelogOptions): ChangelogSelectionReport {
  ...
  const withTitle: PmItem[] = [];
  const missingTitle: PmItem[] = [];
  for (const item of options.items) {
    if (item.title) withTitle.push(item);
...
export function suggestSemverForItems(items: PmItem[]): SemverSuggestion {
  ...
  for (const item of items) {
    ...
    const category = classifyItem(item);
    if (category === "Added") feature++;
    else if (category === "Fixed") fix++;
    else other++;
  }
...
function parseGithubProvenance(
...
function labelItemRef(id: string): string {
  return ` (${escapeMarkdown(id)})`;
}

/** Internal `.toon` blob link under `itemUrlBase`. */
function toonItemRef(item: PmItem, id: string, itemUrlBase: string): string {
...
function githubItemRef(item: PmItem): string | undefined {
